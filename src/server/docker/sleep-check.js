...
const staticThreshold = 5 * msPerMinute;

// docker projects get a day:
const dockerThreshold = 24 * 60 * staticThreshold;
...
export function scheduleContainerCheck() {
  ...
  getAllRunningContainers().forEach(({ image, createdAt, p }) => {
    if (!(p = project(image))) return;
    ...
    const limit = p.app_type === `docker` ? dockerThreshold : staticThreshold;
    if (diff > limit) {
  ...
  const timeout = runInterval * msPerMinute;
  log(`Scheduling next container sleep check for ${date(timeout)}`);
  setTimeout(scheduleContainerCheck, timeout);
}
